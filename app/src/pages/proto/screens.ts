/**
 * Canonical screen map for the migration — the single source of truth shared
 * by the `/proto` preview hub and the router's placeholder route generator.
 *
 * `slug` is the preview path under `/proto/<slug>`; `route` is the eventual
 * real route from MIGRATION_PLAN.md §2.
 *
 * As of Phase 0 every screen has a REAL page module (Agent A placeholders that
 * B/C/D/E replace in place), and `/proto/<slug>` always renders that real
 * component seeded with sample data — so `done` no longer forks to ComingSoon.
 * It now just flags which screens have shipped their final Toss layout (for the
 * hub's "완료" badge). Lanes do not edit this file (Agent A owns it); flip a
 * flag here as each real screen lands.
 */
